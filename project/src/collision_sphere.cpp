...
bool CCollisionSphere::ToMesh(CCollisionMesh* inMesh, bool isExtrusion)
{
	...
	LPDIRECT3DVERTEXBUFFER9 vtx = inMesh->GetVtxBuff();
	LPDIRECT3DINDEXBUFFER9 idx = inMesh->GetIdxBuff();
	...
	for (int nCnt = 0; nCnt < primitive; nCnt++)
	{
			...
			(InOut[0] <= 0 && InOut[1] <= 0 && InOut[2] <= 0)))
		{
			if (isExtrusion)
			{
				...
				if (pos.y < m_extrusionHeight)
				{// メッシュの高さよりプレイヤーの高さのほうが下のとき
					SetPos(D3DXVECTOR3(pos.x, m_extrusionHeight, pos.z));
				}
			}
